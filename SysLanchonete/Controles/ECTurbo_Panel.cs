...
namespace ECTurbo.Controles
{
    public class ECTurbo_Panel : Panel
    {
        ...
        private int vSombraForca = 8;
        [DisplayName("_Sombra Intensidade")]
        public int SombraForca
        {
        ...
        private void EfeitoSombra(Graphics g, Rectangle r)
        {
            if (AtivarSombra == false)
                return;

            int shadowBlur = SombraForca;
            Color shadowColor = Color.FromArgb(128, SombraCor);

            Rectangle shadowRect = new Rectangle(r.X - shadowBlur, r.Y - shadowBlur, r.Width + 2 * shadowBlur, r.Height + 2 * shadowBlur);
